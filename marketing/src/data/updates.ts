...
export const UPDATES: UpdateEntry[] = [
  {
    slug: "2026-w23-a-viewer-for-every-file-and-a-hardened-core",
    title: "A viewer for every file type + a zero-knowledge vault that imports itself",
    published: "2026-06-02",
    week: "Week of June 2, 2026",
    summary:
      "The theme this week: every file, beautifully viewable — and a core hardened to match. 50+ file types now open in custom, fitted, premium viewers (syntax-highlighted code in 100+ languages, live spreadsheets with sheet tabs, interactive 3D models, an EPUB reader, browsable archives, Jupyter notebooks, JSON/YAML/XML trees, and more). The zero-knowledge Vault learned to import itself: drop a .vcf, a password export, a card, or a note and it's parsed in your browser and saved as a structured, end-to-end-encrypted item the server never sees in the clear — plus shareable public Vault links. People and places got smarter (better face grouping, a 'not a person' the model remembers, a faster photo map), search now auto-derives descriptive tags from each file's AI summary, and the job queue grew a crash-safe spine: retries, a dead-letter lane, self-healing reapers, a per-job watchdog, and per-user storage quotas with backpressure.",
    author: "neuthek team",
    tags: ["file-viewers", "vault", "end-to-end-encryption", "people", "photo-map", "search", "tags", "reliability", "security", "quotas"],
    body: {
      found: [
        "Most non-photo files were second-class citizens. A spreadsheet, a 3D model, an EPUB, a Jupyter notebook, a font, or a zip showed up as a generic 'document' icon you had to download to actually open — defeating the point of an in-browser library.",
        "Code preview only covered roughly 40 languages and had no way to search within a long file — you'd scroll a 4,000-line file by hand looking for one symbol.",
        "Spreadsheets only worked for modern .xlsx. Legacy binary .xls (BIFF) and OpenDocument .ods threw an error instead of rendering, so a decade of older workbooks wouldn't open.",
        "Markdown rendered as plain text. CSV files showed as raw comma-soup. JSON, YAML, and XML had no structured tree view — just a wall of text.",
        "The Vault could store structured secure items (Contact / Login / Card / Note), but the only way to create one was to type every field by hand. If you already had a .vcf, a Bitwarden/1Password export, or a notes file, you re-keyed all of it.",
        "There was no way to let someone outside neuthek open a single Vault item — sharing meant they needed an account, so a one-off 'here's the Wi-Fi password' didn't have a clean path.",
        "Face grouping over-split: the same person showed up as several clusters, and there was no durable way to tell the model 'this detection is not a person' (a poster, a mannequin, a reflection) — it would just re-suggest the same bad crop next pass.",
        "The photo map redrew every pin on every pan and zoom, so a library with thousands of geotagged photos felt heavy and stuttered on a trackpad.",
        "Descriptive tags had to be added by hand. The AI already wrote a rich summary of every file, but none of those nouns and scenes flowed into searchable, clickable tags.",
        "The gallery loaded everything in big undifferentiated pages; paging felt abrupt and occasionally double-counted items at the boundary.",
        "The job queue was fragile under failure. A worker that crashed mid-job left the row stuck 'in progress' forever, a transient model error wasn't retried, and there was no dead-letter lane — a single poison file could wedge a slot. Nothing reaped orphaned jobs, and no watchdog killed a job that hung past a sane time budget.",
        "There was no real per-user storage ceiling with graceful backpressure: a runaway import could keep accepting work past what the disk or the plan should allow, instead of refusing cleanly and telling the user why.",
      ],
      fixed: [
        "Built a dedicated, fitted viewer for every major file family and wired a single preview dispatcher to route each file to the right one — so opening a file Just Works regardless of type, with no download.",
        "Code viewer rewritten on a 100+-language grammar set with find-in-file: a real in-file search box with match highlighting and next/previous, word-wrap, copy, and a sticky line-number gutter. Handles very large files without freezing the tab.",
        "Spreadsheets now parse server-side with python-calamine, which reads .xlsx / .xlsm / .xlsb / legacy .xls / .ods alike, and return normalized JSON the grid renders as real cells — sticky A/B/C column headers, sticky 1/2/3 row numbers, and a tab strip to switch sheets.",
        "Markdown renders GitHub-style (headings, tables, task lists, code blocks); CSV/TSV render as a proper grid; JSON / YAML / XML open in a collapsible tree you can expand node by node. Logs render with per-line level coloring (error/warn/info), and HTML opens in a scripts-disabled, sandboxed iframe so a saved web page can't run anything.",
        "Vault import-from-file: drop a .vcf, a password export (.csv/.tsv/.json — browser, Bitwarden, or 1Password shapes), a payment-card JSON, or a .txt/.md note, and the file is parsed entirely in your browser and mapped to the matching secure-item shape (Contact / Login / Card / Note). Only the resulting ciphertext is uploaded; the parser never touches a key, a nonce, the network, or the server.",
        "Shareable public Vault links: seal a single item to a link so someone without an account can open just that item. It stays end-to-end encrypted, scoped to that one item, with no gallery walk and no account requirement — revocable anytime.",
        "Face grouping tuned for fewer false splits, with merge and reassign so you can fold two clusters into one person or move a stray photo to the right name. 'Not a person' is now durable — mark a detection and the model remembers it and stops re-suggesting that crop.",
        "Photo map reworked so pins are virtualized and reused across pan/zoom instead of redrawn from scratch — markedly smoother on large geotagged libraries, with clustering at low zoom and click-to-open at high zoom.",
        "Descriptive tags are auto-derived from each file's AI summary — the salient nouns, scenes, and content types become real tags you can filter by, so 'whiteboard', 'receipt', or 'golden retriever' is one click without you ever typing it.",
        "Gallery paging cleaned up: stable, deterministic pages with no double-count at the boundary and a smoother load as you scroll deeper.",
        "Job queue hardened into a crash-safe spine: failed jobs retry with backoff, terminal failures land in a dead-letter lane instead of wedging a slot, self-healing reapers re-queue jobs orphaned by a crashed worker, and a per-job watchdog kills anything that runs past its time budget so one hung job can't hold a worker hostage.",
        "Per-user storage quotas with backpressure: work is refused cleanly once you hit your ceiling, with a clear 'over quota' signal the UI can act on, so a runaway import stops politely instead of filling the disk.",
        "A security pass across the surface that grew this week: rate limits and timeouts on the new viewer and import endpoints, and audited access controls so every file a viewer fetches is fenced to its owner.",
      ],
      newFeatures: [
        "A viewer for every file type — 50+ types open in custom, fitted, premium viewers: syntax-highlighted code (100+ languages, find-in-file), GitHub-style Markdown, live spreadsheets (xlsx / xls / ods rendered as real cells with sheet tabs), interactive 3D models (STL / OBJ / glTF — render plus a source pane), an EPUB reader, font specimens, browsable archives (preview a file from inside a zip), Jupyter notebooks, JSON / YAML / XML trees, CSV grids, vCard contact cards, iCal event views, level-colored logs, sandboxed HTML, SVG, and HEIC photos — alongside the existing video, audio, PDF, and Office handling.",
        "Vault auto-import — drop a .vcf, a card or ID, a password export, or a note into the Vault and it's parsed client-side and saved as a structured, end-to-end-encrypted item (Contact / Login / Card / Note). The server only ever sees ciphertext.",
        "Shareable public Vault links — hand a single end-to-end-encrypted Vault item to someone without an account via a link, scoped to that one item and revocable anytime.",
        "Smarter people & places — better face grouping, a durable 'not a person', merge and reassign between people, and a faster, smoother photo map.",
        "Tags from AI summaries — descriptive, filterable tags derived automatically from each file's AI summary, so your library becomes searchable by content without manual tagging.",
        "A crash-safe job queue — retries, a dead-letter lane, self-healing reapers, and a per-job watchdog, so the AI pipeline keeps moving through failures instead of silently stalling.",
        "Per-user storage quotas + backpressure — a real ceiling per user with a clean 'over quota' surface, so nothing runs away with your disk.",
      ],
      why:
        "Two ideas drove the week. First: a personal cloud should let you open anything you've stored, right there, fitted to the file — a spreadsheet as cells, a 3D model you can orbit, a notebook with its outputs, an archive you can peek inside — not a download prompt and a generic icon. Getting to 50+ first-class viewers is what makes 'put all your files here' an honest pitch instead of 'put your photos here, and good luck with the rest.' Second: the Vault should be effortless and the engine underneath should be boring in the best way. Auto-import means your existing contacts, logins, and notes become end-to-end-encrypted items in seconds without re-typing — and because the parsing happens in your browser, the zero-knowledge guarantee is untouched: we only ever store ciphertext. The queue, quota, and security work is the unglamorous half — retries, dead-letter, reapers, a watchdog, backpressure, rate limits, timeouts, audited access. None of it shows up in a screenshot, but it's the difference between a demo and something reliable enough to trust your library to. This is reliability and security hardening, built to scale — not a claim that we run a million accounts today; we're a pre-launch personal cloud, and we'd rather earn that line than borrow it.",
      whatTheyDo: [
        "Open anything: click a spreadsheet and read it as live cells with sheet tabs; click a 3D model and orbit it; click a zip and preview a file from inside it; click a notebook, an EPUB, a font, a Markdown file, or a code file and it renders fitted to the type — no download.",
        "Search a long code or text file with find-in-file: open it, hit find, type, and step through matches with next/previous instead of scrolling by hand.",
        "Fill the Vault in seconds: drag a .vcf, a Bitwarden/1Password export, a card JSON, or a notes file into the Vault and each record becomes an encrypted Contact / Login / Card / Note — parsed in your browser, uploaded as ciphertext only.",
        "Share one secret cleanly: create a public link for a single Vault item so someone without an account can open just that, still end-to-end encrypted, and revoke it whenever you want.",
        "Tidy people and places: merge two clusters of the same person, reassign a stray photo, mark a poster or reflection as 'not a person' (and it stays marked), and pan a much smoother photo map.",
        "Find by content for free: the descriptive tags pulled from each file's AI summary are clickable filters, so 'whiteboard' or 'receipt' surfaces the right files without you tagging anything.",
      ],
    },
  },
  {
