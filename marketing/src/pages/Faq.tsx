...
const FAQS: FaqItem[] = [
  ...
  },
  {
    ...
    a: "Partly, and we're precise about which part. The Vault is end-to-end encrypted: anything you put there — files of any type, plus passwords, secure notes, crypto seed phrases and cards/IDs — is encrypted in your browser with a key derived from your master password before it is uploaded. The server stores only ciphertext and cannot read it, and no AI ever touches it. You can drop a .vcf, a password export, a card, or a note in and it's parsed client-side into a structured item, and you can share a single vault item — either sealed to another neuthek account's key or via a public link scoped to that one item — staying end-to-end encrypted and revocable anytime. The normal Drive is NOT end-to-end encrypted: there the server holds the keys so it can run the AI features you've consented to (semantic search, captions, face grouping) — those lose capability the moment their inputs become ciphertext we can't read, which is the deliberate trade-off. So we won't call the whole product \"end-to-end encrypted,\" because it isn't — only the Vault is. Overstating that is exactly what the FTC went after Zoom for in 2020.",
  },
  {
    topic: "Privacy",
    id: "vault-auto-import",
    q: "How does the encrypted Vault's auto-import work, and is it really zero-knowledge?",
    a: "Yes, it stays zero-knowledge. The Vault is an end-to-end-encrypted store for files and structured secure items (Contact / Login / Card / Note). Auto-import lets you drop a file in instead of typing fields by hand: a .vcf becomes a Contact, a password export (.csv / .tsv / .json from a browser, Bitwarden, or 1Password) becomes one Login per entry, a payment-card JSON becomes a Card, and a .txt / .md note becomes a Note. The parsing happens entirely in your browser — the import code never touches an encryption key, a nonce, the network, or the server. Each mapped record is then encrypted on your device with a key derived from your master password, and only the resulting ciphertext is uploaded. The server never sees the plaintext, so the zero-knowledge guarantee is untouched. You can also share a single Vault item via a public link scoped to just that item, still end-to-end encrypted and revocable anytime.",
  },
  {
    topic: "Privacy",
  ...
  // ---- File support ----
  {
    ...
    a: "Over 50 file types open in a custom, fitted, in-browser viewer — no download needed. Photos: JPEG, PNG, HEIC, HEIF, WebP, AVIF, SVG, animated GIF (passthrough — frames preserved), and camera RAW (Nikon NEF, Canon CR2, Sony ARW, Adobe DNG, Fuji RAF, Olympus ORF, Panasonic RW2, Pentax PEF) via LibRaw. Video: MP4, MOV, WebM, MKV with frame-grab thumbnails; audio gets an inline player. Documents: PDF (with OCR-extracted text into the search index) and Office files. Code: 100+ languages with syntax highlighting and find-in-file. Data & text: GitHub-style Markdown, JSON / YAML / XML as a collapsible tree, CSV / TSV grids, and level-colored logs. Spreadsheets: xlsx, xls (legacy BIFF), xlsm, xlsb and ods rendered as live cells with sheet tabs. Plus interactive 3D models (STL / OBJ / glTF / GLB), an EPUB reader, font specimens (.ttf / .otf / .woff), vCard contact cards, iCal event views, browsable archives (.zip / .tar / .7z / .rar — preview a file from inside one), Jupyter notebooks, and sandboxed (scripts-disabled) HTML.",
  },
  {
    topic: "File support",
    id: "in-browser-file-viewers",
    q: "Can I open documents, spreadsheets, code, and 3D models without downloading them?",
    a: "Yes — that's a core feature. neuthek routes each file to a viewer fitted to its type, all rendered in the browser. Code opens with syntax highlighting for 100+ languages plus find-in-file (an in-file search box with match highlighting and next/previous). Spreadsheets (xlsx / xls / ods) render as real cells with sticky headers and a tab strip to switch sheets. 3D models (STL / OBJ / glTF) open in an orbit-able WebGL viewer with a source pane for the text formats. Markdown renders GitHub-style; JSON / YAML / XML open as a collapsible tree; CSV renders as a grid; EPUB books, fonts, vCards, iCal events, Jupyter notebooks, and the contents of archives all have dedicated viewers. Every viewer fetches through an access-controlled route fenced to your account, server-side parsers run under timeouts and rate limits, and HTML is rendered with scripts disabled.",
  },
  {
    topic: "File support",
  ...
  // ---- Migration ----
  {
    ...
    a: "Google Drive sync is fully wired today: it uses OAuth 2.0 with PKCE and the read-only drive.readonly scope — neuthek can never write back — and it covers Google Photos exports plus general Drive files. Connectors for Dropbox, iCloud Drive, Proton Drive, and MEGA are scaffolded in the same cloud-sync framework (Dropbox via OAuth read-only; iCloud via Apple-ID with HSA-2 push + SMS fallback; Proton and MEGA via rclone with email + password) and are being finished ahead of launch. For Proton and MEGA the trade-off is the same one you'd make installing their desktop sync clients: they keep server-side E2E inside their products, but once neuthek ingests we hold plaintext so AI can run on it. In the meantime you can always export any provider to a local folder and upload directly. OneDrive isn't planned — Microsoft's Personal SDK license terms made it the wrong fit.",
  },
  {
    topic: "Migration",
