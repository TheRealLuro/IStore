...
# --------------------------------------------------------------------- #
# Marketing-site waitlist (real signup endpoint).
# --------------------------------------------------------------------- #
# Closed enum on the FE — keep the server-side allow-list in sync with
# the <select> options on marketing/src/pages/Waitlist.tsx. Anything
# else lands as `other`.
WAITLIST_USE_CASES: set[str] = {
    "personal",
    "family",
    "creative",
    "research",
...
class WaitlistSignupCreate(BaseModel):
    email: EmailStr
    use_case: str = Field(default="personal", max_length=32)

    @model_validator(mode="after")
    def _normalize(self) -> "WaitlistSignupCreate":
        # Pydantic lower-cases the local-part for us; force the domain too
        # so the unique index treats Bob@Example.com == bob@example.com.
        self.email = self.email.lower().strip()  # type: ignore[assignment]
        if self.use_case not in WAITLIST_USE_CASES:
            self.use_case = "other"
        return self


class WaitlistSignupResult(BaseModel):
    """The public response is intentionally minimal — never echo back the
    email or any other field the caller could enumerate against."""
